...
## Live Validation With a Real API Key (2026-07-20, follow-up session)

The user supplied a real Anthropic API key (entered directly by the user, never handled by the agent) and drove the full loop live:

- **Verified working with real generation**: the morning follow-up (Call 1) and missed-check-in acknowledgment (Call 2) both produced genuinely reactive, well-targeted questions referencing the user's actual input — not templated text. Full morning → night loop completed with a real streak update (0 → 1). Full FR-010 acknowledgment → morning handoff completed end to end.
- **Two more real bugs found and fixed, both in the FR-010 acknowledgment → morning handoff path**:
  1. `src/lib/heatmap.ts` rendered today's square as "incomplete" (red) as soon as a commitment was set, even though the night check-in wasn't due yet — `computeHeatmap` didn't exclude today's still-`"pending"` status the way `computeStreak` already does. Fixed: today with `completionStatus: "pending"` now renders `"no-entry"` until actually resolved. Regression test added in `tests/unit/heatmap.test.ts`.
  2. After answering the acknowledgment prompt, clicking "Continue"/submitting the morning avoidance **silently did nothing** — no loading state, no error, the button just didn't respond. Root cause: `submitAcknowledgment` writes a `DailyEntry` for today containing only the ack fields (no `avoidance`/`commitment` yet), but `deriveCheckinPhase` classified any entry without a commitment as `"morning-in-progress"` rather than `"no-entry"`, so `canStartMorningCheckin`'s guard inside `submitAvoidance` silently blocked the call. Separately, `submitAvoidance` built its new `DailyEntry` from scratch instead of merging onto the existing one, which would have silently wiped the acknowledgment fields (`acknowledgedMissedPrior`/`acknowledgmentResponse`) once the write succeeded. Fixed both: `deriveCheckinPhase` now checks for `entry.avoidance` before falling into `"morning-in-progress"`, and `submitAvoidance` spreads the existing entry as its base. Regression test added in `tests/unit/checkinStateMachine.test.ts` and a new end-to-end hook test in `tests/unit/useCheckinFlow.test.ts` covering the full acknowledgment → morning handoff, asserting the ack fields survive the write.
- Both fixes were re-verified live with the real API key: the status label now correctly reads "Start your morning check-in" (not "Continue"), submitting proceeds normally, and the stored `DailyEntry` retains both the acknowledgment fields and the new avoidance/follow-up/commitment data.
